...
*/
rt_err_t rt_usbh_cdc_get_line_code(struct uhintf* intf, cdc_line_coding_t *line_coding)
{
    ...
    if(rt_usb_hcd_pipe_xfer(device->hcd, device->pipe_ep0_in, line_coding->buffer,
        setup.wLength, timeout) != setup.wLength) {
        return -RT_EIO;
    }
 ...
 * @return the error code, RT_EOK on successfully.
*/
rt_err_t rt_usbh_cdc_set_line_code(struct uhintf* intf, cdc_line_coding_t *line_coding)
{
    struct uinstance   *device;    
    struct urequest     setup;
    int                 timeout = USB_TIMEOUT_BASIC;
 ...
 * @return the error code, RT_EOK on successfully.
*/
rt_err_t rt_usbh_cdc_set_control_line_state(struct uhintf* intf, uint32_t wValue)
{
    struct uinstance   *device;    
    struct urequest     setup;
    int                 timeout = USB_TIMEOUT_BASIC;
